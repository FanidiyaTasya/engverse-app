export default class ListeningResultPage {
  weakPrediction = null;

  async render() {
    const timeSpent = localStorage.getItem("listening_time_spent") || "00:00";
    const raw = localStorage.getItem("listening_result");
    ...
    let html = `
      ...
      <section class="container mx-auto px-4 py-8 max-w-4xl">
        <div class="bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-t-xl text-center py-8">
          <h2 class="text-2xl font-bold mb-2">Listening Practice Result</h2>
  ...
  async afterRender() {
    ...
    if (correct > 0 && correct < total) {
      const se = this.#calcError(data, "Short Conversation");
      const ee = this.#calcError(data, "Extended Conversation");
      const te = this.#calcError(data, "Talk");
      this.weakPrediction = await this.#predictWeakPart([se, ee, te]);
    } else {
    ...
    document
      ...
      ?.addEventListener("click", () => {
        localStorage.removeItem("listening_result");
        localStorage.removeItem("listening_time_spent");
      });
    document.getElementById("try-again")?.addEventListener("click", () => {
      localStorage.removeItem("listening_result");
      localStorage.removeItem("listening_time_spent");
